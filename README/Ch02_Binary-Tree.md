...
- 구현 할 메서드
  - BinaryTree._insertNode() : 재귀로 트리를 순회하며 노드 추가(_는 내부 사용을 의미)
  - BinaryTree.insert() : 노드 추가
  - BinaryTree._preOrderTraverseNode() : 재귀로 트리를 순회하며 전위 순회(내부 사용)
  - BinaryTree.preOrderTraverse() :  전위 순회하며 노드 출력
  - BinaryTree._inOrderTraverseNode() : 재귀로 트리를 순회하며 중위 순회(내부 사용)
  - BinaryTree.inOrderTraverse() : 중위 순회하며 노드 출력
  - BinaryTree._postOrderTraverseNode() : 재귀로 트리를 순회하며 후위 순회(내부 사용)
  - BinaryTree.postOrderTraverse() : 후위 순회하며 노드 출력
  - BinaryTree.levelOrderTraverse() : 층별 순회하며 노드 출력

## 이진 트리 순회 구현하기
```javascript
/* Queue 객체 추가( 층별 순회를 위한 큐 )*/ 
